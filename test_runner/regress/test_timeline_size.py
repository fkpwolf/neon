...
def test_timeline_initial_logical_size_calculation_cancellation(neon_env_builder: NeonEnvBuilder):
    ...
    env.pageserver.start(
        extra_env_vars={"FAILPOINTS": "timeline-calculate-physical-size-pause=pause"}
    )
    ...
    client.configure_failpoints(("timeline-calculate-physical-size-check-dir-exists", "return"))
    client.configure_failpoints(("timeline-calculate-physical-size-pause", "off"))
    ...
    # message emitted by the code behind failpoint "timeline-calculate-physical-size-check-dir-exists"
